...
namespace Pathfinding
{
    public class Pathfinder
    {
                ...
                foreach (Node neighbour in GetNeighbours(currentNode,true))
                {
                    if (!closedSet.Contains(neighbour))
                    {
                        ...
                        if (newMovementCostToNeighbour < neighbour.gCost || !openSet.Contains(neighbour))
                        {
                            ...
                            neighbour.hCost = GetDistance(neighbour, target);//==ManHaDunDistance(neighbour, target)
        ...
        private int ManHaDunDistance(Node posA, Node posB)
        {
            int distX = Mathf.Abs(posA.x - posB.x);
            int distZ = Mathf.Abs(posA.z - posB.z);
            int distY = Mathf.Abs(posA.y - posB.y);
            return 10 * distX + 10 * (distZ - distX) + 10 * distY;
        }
    }
}
